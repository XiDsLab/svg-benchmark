...
normal_gen_cho2<-setdiff(rownames(info_gene)[which((info_gene$gener_gen==0&((info_gene$max_spot-info_gene$min_spot)<0.2)&info_gene$global_mean>0.01))],DE_gen)

DE_gen<-sample(DE_gen,size=de_potion*ngen,replace=FALSE)
normal_gen1<-sample(normal_gen_cho1,size=6000,replace = FALSE)
normal_gen2<-sample(normal_gen_cho2,size=1000,replace = FALSE)
normal_gene<-c(normal_gen1,normal_gen2)
...
save(counts,DE_gen,info,info_gene,kind,file="simcounts_me.RData")
write.csv(counts,file = "simcounts_me.csv")